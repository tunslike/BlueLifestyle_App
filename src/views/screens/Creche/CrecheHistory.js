...
import {  StyleSheet, 
  ...
  // init app screen
const CrecheHistory = ({navigation}) => {
  ...
  const LoadPendingOrders = () => {
    ...
    LoadPendingCrecheOrder("False")
  }

  const LoadCompletedOrders = () => {
    setPendingStatus(false)
    setCompletedStatus(true)
    LoadPendingCrecheOrder("True")
  }

  // function to load providers
  const LoadPendingCrecheOrder = (status) => {
    ...
    axios.post(APIBaseUrl.developmentUrl + 'orders/order/FetchCrecheOrders', data, {
      headers: {
        'JWTToken': token
      }
    })
      ...
      setIsLoading(false)
        ...
        if(response.data.errorCode == '000') {
             ...
             if(status == "False") {
                ...
                for (let i= 0; i < response.data.crecheOrders.length; i++) {
                    if (response.data.crecheOrders[i].isTreated == "False") {
                        filteredData = [...filteredData, response.data.crecheOrders[i]];
                  }
                }
                ...
                for (let i= 0; i < response.data.crecheOrders.length; i++) {
                    if (response.data.crecheOrders[i].isTreated == "True") {
                        filteredData = [...filteredData, response.data.crecheOrders[i]];
                  }
                }
...
// end of function
   ...
   useEffect(() => {
    ...
    LoadPendingCrecheOrder(orderStatus);
  ...
  function renderHeaderContent() {
    return (
        ...
        <View style={styles.headerDisplay}>
          <Text style={styles.titleName}>Creche Orders</Text>
          ...
          <View style={styles.officeDetails}>
            ...
            <Text style={styles.business}>VIEW BOOKING HISTORY</Text>
          </View>
