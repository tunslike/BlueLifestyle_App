...
const GymHistory = ({navigation}) => {
  ...
  const LoadPendingOrders = () => {
    ...
    LoadPendingGymOrder("False")
  }

  const LoadCompletedOrders = () => {
    setPendingStatus(false)
    setCompletedStatus(true)
    LoadPendingGymOrder("True")
  }

  // function to load providers
  const LoadPendingGymOrder = (status) => {
    ...
    axios.post(APIBaseUrl.developmentUrl + 'orders/order/FetchGymOrders', data, {
      headers: {
        'JWTToken': token
      }
    })
      ...
      setIsLoading(false)
        ...
        if(response.data.errorCode == '000') {
             ...
             if(status == "False") {
                ...
                for (let i= 0; i < response.data.gymOrders.length; i++) {
                    if (response.data.gymOrders[i].isTreated == "False") {
                        filteredData = [...filteredData, response.data.gymOrders[i]];
                  }
                }
                ...
                for (let i= 0; i < response.data.gymOrders.length; i++) {
                    if (response.data.gymOrders[i].isTreated == "True") {
                        filteredData = [...filteredData, response.data.gymOrders[i]];
                  }
                }
...
// end of function
   ...
   useEffect(() => {
    ...
    LoadPendingGymOrder(orderStatus);
  ...
  function renderHeaderContent() {
    return (
        ...
        <View style={styles.headerDisplay}>
          <Text style={styles.titleName}>Gym Orders</Text>
